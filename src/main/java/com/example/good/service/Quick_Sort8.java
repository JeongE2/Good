...
public class Quick_Sort8 {

    public ArrayList<FoodEntity> quickSort8(ArrayList<FoodEntity> li) {
        ...
        double pivot = li.get(li.size() / 2).getNutr_cont8();
        FoodEntity pivot_Food = li.get(li.size() / 2);
        li.remove(li.size() / 2);

        for (int i = 0; i < li.size(); i++) {
            if (li.get(i).getNutr_cont8() >= pivot) {
        ...
        left = quickSort8(left);
        right = quickSort8(right);
