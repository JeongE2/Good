...
public class Quick_Sort5 {

    public ArrayList<FoodEntity> quickSort5(ArrayList<FoodEntity> li) {
        if (li.size() < 2) {
            return li;
        }
        ...
        double pivot = li.get(li.size() / 2).getNutr_cont5();
        FoodEntity pivot_Food = li.get(li.size() / 2);
        li.remove(li.size() / 2);

        for (int i = 0; i < li.size(); i++) {
            if (li.get(i).getNutr_cont5() >= pivot) {
        ...
        left = quickSort5(left);
        right = quickSort5(right);
