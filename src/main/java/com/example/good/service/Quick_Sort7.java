...
public class Quick_Sort7 {

    public ArrayList<FoodEntity> quickSort7(ArrayList<FoodEntity> li) {
        ...
        double pivot = li.get(li.size() / 2).getNutr_cont7();
        FoodEntity pivot_Food = li.get(li.size() / 2);
        li.remove(li.size() / 2);

        for (int i = 0; i < li.size(); i++) {
            if (li.get(i).getNutr_cont7() >= pivot) {
        ...
        left = quickSort7(left);
        right = quickSort7(right);
